...
function upaymentGateway($data){
	...
	$user = selectDB("users","`id` = '{$data["userId"]}'");
	$orderId = date("Ymd").rand(0000,9999).time();
	$postBody = array(
		'language' => 'en',
		'order[id]' => $orderId,
		'order[currency]' => 'KWD',
		'order[amount]' => (string)$data["price"],
		'order[description]' => "order for {$user[0]["fName"]} {$user[0]["lName"]}, {$user[0]["phone"]}",
		'reference[id]' => $orderId,
		'customer[name]' => "{$user[0]["fName"]} {$user[0]["lName"]}",
		'customer[email]' => "{$user[0]["email"]}",
		'customer[mobile]' => "{$user[0]["phone"]}",
		'returnUrl' => 'https://coeoapp.com/index.php',
		'cancelUrl' => 'https://coeoapp.com/index.php',
		'notificationUrl' => 'https://coeoapp.com/index.php',
		'extraMerchantData[0][amount]' => (string)$data["price"],
		...
		'extraMerchantData[0][ibanNumber]' => "{$settings[0]["mainIban"]}",
		);
