...
if( isset($_FILES['logo']) AND is_uploaded_file($_FILES['logo']['tmp_name']) ){
	...
	?>
	<script>
	window.location.href = "?page=images&id=<?php echo $_GET['id'] ?>";
	</script>
	<?php
}
...
<div class="pull-left">
<h6 class="panel-title txt-dark">Upload Photo</h6>
</div>
...
<form action="?page=images&id=<?php echo $_GET["id"] ?>" method="post" enctype="multipart/form-data">
...
?>
<img src="logos/<?php echo $row["imageurl"] ?>" style="width:50px;height:50px">
<?php
}
?>
</td>
...
<a href="?page=images&id=<?php echo $_GET["id"] ?>&<?php echo $action[$i] . $row["id"] ?>" style="margin:3px"><i class="<?php echo $icon[$i] ?>"></i></a>
